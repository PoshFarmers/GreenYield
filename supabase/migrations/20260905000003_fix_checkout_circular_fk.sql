-- Fix: Circular foreign key dependency between orders and payment.
-- orders.payment_id references payment(id)
-- payment.order_id references orders(id)
-- We must insert orders with a null payment_id, then insert payment,
-- then update orders with the payment_id.
...
begin
  ...
  loop
    ...
    -- 1. Insert orders WITH NULL payment_id to avoid FK violation
    insert into orders (
    ...
    -- 2. Insert payment referencing the order
