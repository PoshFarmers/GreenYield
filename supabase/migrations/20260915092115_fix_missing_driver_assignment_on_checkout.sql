-- ============================================================
-- Bug: 20260914000000_add_order_date.sql redefined place_checkout
-- to add p_order_date, but its function body was copied from a
-- version predating 20260913105735_auto_assign_nearest_driver.sql
-- — it dropped the `perform assign_nearest_driver(v_order_id);`
-- call. Every order placed since that migration ran has been
-- silently stuck at status = 'placed' with no `delivery` /
-- `delivery_assignment` row:
--   - Driver Deliveries screen queries delivery_assignment ->
--     nothing to show.
--   - Driver Calendar screen (driver_schedule_service.dart) joins
--     delivery_assignment too -> nothing to show.
--   - trg_notify_delivery_assignment only fires on a
--     delivery_assignment INSERT -> no notification ever sent.
--
-- Fix: reinstate the call in place_checkout (same 5-arg signature
-- introduced by 20260914000000), and backfill orders that were
-- placed since that regression landed.
-- ============================================================
...
begin
  ...
  loop
    ...
    -- *** THE FIX ***
    -- This call was dropped when 20260914000000_add_order_date.sql
    -- redefined place_checkout. Without it, no `delivery` /
    -- `delivery_assignment` row is ever created, so the order never
    -- reaches a driver's Calendar or Deliveries screen, and the
    -- delivery_assignment INSERT trigger that sends notifications
    -- never fires.
...
-- ============================================================
-- Backfill: assign a driver to every order placed since the
-- regression (20260914000000) that is still stuck at 'placed'
-- with no delivery — including the 17 Sept 2026 order reported
-- as missing.
-- ============================================================
do $$
declare
...
begin
  ...
  loop
    ...
    exception when others then
      ...
      -- (e.g. the farmer has no saved location yet).
