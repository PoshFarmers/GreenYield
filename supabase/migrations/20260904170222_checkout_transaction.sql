-- Online checkout transaction. Cart remains PowerSync-backed, but order
-- creation is authoritative and atomic in Supabase.
...
create table checkout_request (
  ...
  unique (buyer_profile_id, id)
);

alter table checkout_request enable row level security;

create policy "checkout_request_select_own" on checkout_request
